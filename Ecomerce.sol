...
contract Ecomerce{
    ...
    modifier isNotDestroyed{
        require(!destroyed, "Contract doesnot exist");
        _;
    }
    ...
    function registerProduct(string memory _title, string memory _desc, uint _price) public isNotDestroyed{
        require(_price > 0 , "Price should be greter than zero");
        Product memory tempProduct;
        tempProduct.titile = _title;
    ...
    function buy(uint _productId) payable public isNotDestroyed{
        require(products[_productId-1].price == msg.value, "Please pay thw exact price");
